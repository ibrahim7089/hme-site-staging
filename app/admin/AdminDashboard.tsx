...
import { Check, ChevronRight, FileCheck2, History, LogOut, Plus, RefreshCw, RotateCcw, Send, ShieldCheck, Users, X } from 'lucide-react'
...
export default function AdminDashboard({ user, permissions }: { user: CmsUser; permissions: CmsPermission[] }) {
  ...
  async function save() {
    ...
    try {
      ...
      setSelected(item); flash(selected ? 'Draft updated' : 'Draft created'); await load()
    } catch (caught) {
      setError(caught instanceof SyntaxError ? 'JSON is invalid. Check commas and quotation marks.' : caught instanceof Error ? caught.message : 'Save failed')
  ...
  async function act(action: string) {
    ...
    if (!window.confirm(`${action[0].toUpperCase() + action.slice(1)} version ${selected.version}?`)) return
    setBusy(true); setError('')
    try {
      const result = await api(`/api/admin/publishing/${selected.id}/${action}`, { method: 'POST', body })
      setSelected(result.item || result); flash(result.scheduled ? 'Publishing has been scheduled' : `Completed: ${action}`); await load()
  ...
  return <div className={`admin-root ${styles.adminShell}`}>
    <aside className={styles.sidebar}>
      <div className={styles.brand}><Image src="/logo.png" alt="HME" width={82} height={49} priority /><span><strong>Website Admin</strong><small>Publishing operations</small></span></div>
      <nav>
        <button className={section === 'publishing' ? styles.navActive : ''} onClick={() => setSection('publishing')}><FileCheck2 size={19} /> Publishing</button>
    ...
    <main className={styles.main}>
      <header className={styles.topbar}><div><p className={styles.kicker}>HME content operations</p><h1>{section === 'publishing' ? 'Website publishing' : 'Users & roles'}</h1></div><div className={styles.secure}><ShieldCheck size={18} /> Maker-checker enabled</div></header>
      ...
      {section === 'users' ? <section className={styles.userGrid}>
        ...
        <section className={styles.workspace}>
          <div className={styles.listPanel}>
            <div className={styles.listTools}><select value={status} onChange={(e) => setStatus(e.target.value)}><option value="">All statuses</option>{statuses.map((entry) => <option key={entry}>{entry}</option>)}</select><button title="Refresh" onClick={load}><RefreshCw size={17} /></button>{can('publishing.create') && <button className={styles.newButton} onClick={() => setSelected(null)}><Plus size={17} /> New</button>}</div>
            <div className={styles.itemList}>{items.length === 0 && <div className={styles.empty}>No records yet.<br />Create the first draft.</div>}{items.map((item) => <button key={item.id} className={selected?.id === item.id ? styles.itemActive : styles.item} onClick={() => setSelected(item)}><span><strong>Version {item.version}</strong><small>{item.created_by_name} · {date(item.updated_at)}</small></span><em className={styles[`status${item.status}`]}>{item.status}</em><ChevronRight size={17} /></button>)}</div>
          </div>

          <div className={styles.editorPanel}>
            <div className={styles.editorHead}><div><p className={styles.kicker}>{labels[type]}</p><h2>{selected ? `Version ${selected.version}` : 'New draft'}</h2></div>{selected && <em className={styles[`status${selected.status}`]}>{selected.status}</em>}</div>
            {selected?.rejection_reason && <div className={styles.rejection}>Rejected: {selected.rejection_reason}</div>}
            <label>Content (validated JSON)<textarea className={styles.codeEditor} value={editor} onChange={(e) => setEditor(e.target.value)} readOnly={!editable} spellCheck={false} /></label>
            <div className={styles.twoColumns}><label>Change note<input value={note} onChange={(e) => setNote(e.target.value)} readOnly={!editable} placeholder="What changed and why?" /></label><label>Schedule (optional)<input type="datetime-local" value={schedule} onChange={(e) => setSchedule(e.target.value)} disabled={!editable} /></label></div>
            <div className={styles.actions}>
              {editable && <button className={styles.primaryButton} onClick={save} disabled={busy}>{selected ? 'Save changes' : 'Create draft'}</button>}
              {selected?.status === 'DRAFT' && selected.created_by_user_id === user.id && can('publishing.submit') && <button onClick={() => act('submit')} disabled={busy}><Send size={17} /> Submit</button>}
              {selected?.status === 'PENDING' && can('publishing.approve') && <><button className={styles.approveButton} onClick={() => act('approve')} disabled={busy}><Check size={17} /> Approve</button><button className={styles.rejectButton} onClick={() => act('reject')} disabled={busy}><X size={17} /> Reject</button></>}
              {selected?.status === 'APPROVED' && can('publishing.publish') && <button className={styles.approveButton} onClick={() => act('publish')} disabled={busy}><Check size={17} /> {selected.scheduled_for ? 'Confirm schedule' : 'Publish now'}</button>}
              {selected && ['PUBLISHED', 'ARCHIVED'].includes(selected.status) && can('publishing.create') && <button onClick={() => act('rollback')} disabled={busy}><RotateCcw size={17} /> Rollback draft</button>}
            </div>
          </div>

          <aside className={styles.auditPanel}><div className={styles.panelHead}><p className={styles.kicker}>Immutable log</p><h2><History size={18} /> Audit trail</h2></div>{!selected && <div className={styles.empty}>Select an item to view history.</div>}{events.map((event) => <div className={styles.event} key={event.id}><i /><span><strong>{event.action.replaceAll('_', ' ')}</strong><small>{event.actor_name || 'System'} · {date(event.created_at)}</small>{event.note && <p>{event.note}</p>}</span></div>)}</aside>
        </section>
      </>}
    </main>
  </div>
