...
import { Banknote, Clock3, MapPin, Navigation, Phone } from "lucide-react";
...
export default function BranchDirectory({
  ...
  const embedUrl = active
    ? `https://maps.google.com/maps?hl=en&q=${encodeURIComponent(active.address)}&z=15&output=embed`
    : null;

  return (
    <div className="mt-10 grid gap-6 lg:grid-cols-[1fr_1.15fr]">
      <div className="relative order-2 h-[360px] overflow-hidden rounded-card border border-line bg-cloud lg:order-1 lg:sticky lg:top-24 lg:h-[560px]">
        {embedUrl && active ? (
          <>
            <iframe
              key={embedUrl}
            ...
            />
            <div className="absolute inset-x-3 bottom-3 flex items-center justify-between gap-3 rounded-xl bg-white/95 p-3 shadow-deep backdrop-blur">
              ...
              <a href={active.mapsUrl} target="_blank" rel="noopener noreferrer"
                className="inline-flex flex-none items-center gap-1.5 rounded-lg bg-brand-blue px-3 py-2 text-xs font-bold text-white">
                <Navigation className="h-3.5 w-3.5" /> Open Map
              </a>
            </div>
          </>
        ) : (
          <div className="grid h-full place-items-center p-6 text-center text-sm text-slate2">
            ...
            return (
              ...
              >
                ...
                <div className="mt-4 flex flex-wrap gap-2 border-t border-line pt-4">
                  ...
                  >
                    <MapPin className="h-3.5 w-3.5" /> {isSelected ? "Shown on Map" : "Show on Map"}
                  </button>
