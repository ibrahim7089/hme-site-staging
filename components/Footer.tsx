...
import { getPublishedGlobalContent } from "@/lib/cms";

export default async function Footer() {
  const global = await getPublishedGlobalContent();
  const socialLinks = [
    { label: "Facebook", icon: FaFacebookF, href: global.facebookUrl },
    { label: "Instagram", icon: FaInstagram, href: global.instagramUrl },
    { label: "TikTok", icon: FaTiktok, href: global.tiktokUrl },
    { label: "LinkedIn", icon: FaLinkedinIn, href: global.linkedinUrl },
      ...
      <div className="wrap grid gap-5 py-6 md:grid-cols-[auto_1fr] md:items-center md:gap-10">
        ...
        <div className="min-w-0">
          ...
          <div className="flex flex-col items-center justify-between gap-4 border-t border-white/10 pt-5 text-center text-xs sm:flex-row sm:text-left">
            <span>{global.footerCopyright}</span>
